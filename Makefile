...
$(AUDIO_DIR)/$(AUDIO_LIB) :
	...
	UNAME="Linux" \
		...
		GLU_CFLAGS="" \
		V=1 \
		OPTFLAGS="$(OPT_LEVEL) $(DEBUG_LEVEL) -s SIDE_MODULE=1 -s -DEMSCRIPTEN=1 -DNO_FILTER_THREAD=1" \
		all
...
$(RSP_DIR)/$(RSP_LIB) :
	...
	emmake make \
		...
		GLU_CFLAGS="" \
		V=1 \
		OPTFLAGS="$(OPT_LEVEL) $(DEBUG_LEVEL) -s SIDE_MODULE=1 -s -DEMSCRIPTEN=1 -DVIDEO_HLE_ALLOWED=1" \
		all
